...
const prefix = '*';
const token = `NzA1OTA3NjQ2NDgxNDMyNjA4.XsAO3A.6s-YWmdGPv0BVfpCXipaEaWLosw`;
const r = "RANDOM";
...
bot.on('message', message => {
    ...
    if (cmd === 'clear' || cmd === 'purge'){
        ...
        msg.channel.send(`**:white_check_mark: Successfully deleted \`${args[0]} messages\`**`).then(m => m.delete({ timeout: 8000 }));
    }
    if(cmd === 'kick'){
